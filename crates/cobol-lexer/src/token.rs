...
impl TokenKind {
    ...
    pub fn from_keyword(word: &str) -> Option<TokenKind> {
        ...
        match upper.as_str() {
            ...
            "TERMINAL" => Some(TokenKind::Terminal),
            "SYMBOLIC" => Some(TokenKind::Symbolic),
            "SUB-QUEUE-1" => Some(TokenKind::SubQueue1),
            "SUB-QUEUE-2" => Some(TokenKind::SubQueue2),
            "SUB-QUEUE-3" => Some(TokenKind::SubQueue3),
            "QUEUE" => Some(TokenKind::Queue),
            "DESTINATION" => Some(TokenKind::Destination),
            "TABLE" => Some(TokenKind::Table),
            "ERROR-KEY" => Some(TokenKind::ErrorKey),
            "STATUS-KEY" => Some(TokenKind::StatusKey),
            "END-KEY" => Some(TokenKind::EndKey),
            "TEXT" => Some(TokenKind::Text),
            "LENGTH" => Some(TokenKind::Length),
            "DATE" => Some(TokenKind::DateKw),
            "TIME" => Some(TokenKind::TimeKw),
            "EMI" => Some(TokenKind::Emi),
            "EGI" => Some(TokenKind::Egi),
            "ESI" => Some(TokenKind::Esi),
