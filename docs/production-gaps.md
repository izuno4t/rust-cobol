...
入手先: GnuCOBOL SourceForge の nist ディレクトリ
(`newcob.val.Z`)。テストプログラムは全て固定形式。

段階的に以下のモジュール順で通過率を上げる。

1. **NC (Nucleus)** — 約95プログラム。
   COBOL核文法の網羅テスト。最優先
2. **SM (Source Manipulation)** — 約17プログラム。
   COPY文テスト
3. **IC (Inter-program Communication)** — 約25プログラム。
   CALL文テスト
4. **SQ (Sequential I/O)** — 順編成ファイルI/Oテスト
5. **IF (Intrinsic Functions)** — 組み込み関数テスト。
   Phase 2の検証に直結
6. **IX (Indexed I/O)** — 索引編成ファイルI/Oテスト
7. **RL (Relative I/O)** — 相対編成ファイルI/Oテスト
8. **ST (SORT/MERGE)** — ソート・マージ文テスト
9. **RW (Report Writer)** — REPORT SECTIONテスト
10. **DB (Debugging)** — デバッグ機能テスト
11. **SG (Segmentation)** — セグメント機能テスト
12. **OB (Obsolete)** — 廃止予定機能テスト（低優先）

テスト実行基盤として以下が必要:

- `newcob.val` からのテストプログラム抽出スクリプト
  （EXEC85はCOBOLプログラムなのでPerlで代替実装）
- テスト実行・結果集計の自動化（Makefile/CI統合）
- 通過率のダッシュボード（モジュール別PASS/FAIL/INSPECT）
...
1. **パースエラーの位置情報改善** —
   行番号・列番号・該当行の表示。
   現在のDiagnosticReporterの出力を
   rustc風の`^^^`アンダーライン付きに改善
2. **型エラーの具体的なメッセージ** —
   「型が合わない」ではなく
   「PIC 9(5) に PIC X(10) は移送できない」レベルの詳細
3. **警告レベルの導入** —
   致命的エラー / 警告 / 情報の区別。
   `-W` フラグによる制御
4. **複数エラーの継続報告** —
   最初のエラーで停止せず、可能な限り多くの
   エラーを1回のコンパイルで報告
   （現在部分的に対応済み、網羅性の向上）
5. **エラーコード体系** —
   `COBC-E001` のような一意のエラーコードを付与。
   ドキュメントからの参照を可能にする

### Phase 9: 実業務プログラムでの検証

NIST以外の実践的な検証。

1. **GnuCOBOL互換テスト** —
   GnuCOBOLのテストスイート（`tests/testsuite.src`）の
   主要ケースを移植して通過率を測定
2. **オープンソースCOBOLプログラムの動作検証** —
   GitHub上の実在するCOBOLプログラム
   （バッチ処理、帳票、ファイル変換等）をコンパイル・実行
3. **性能ベンチマーク** —
   GnuCOBOLとの実行速度比較。
   ファイルI/O、算術演算、文字列操作の各カテゴリ
4. **ドキュメント整備** —
   ユーザ向けのコンパイラ使用方法、
   対応COBOL規格一覧、既知の制限事項
